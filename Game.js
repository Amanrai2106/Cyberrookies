...
const snakeSound = new Audio('snakes.mp3');
...
export class Game {
  ...
  handlePlayerMove() {
    ...
    diceImg.src = "images/rolling.gif";
  
    
    document.querySelector(".dice .dice-img").innerHTML = "";
        ...
        if (player.position == 0 && player.rolledNumber == 6) {
          ...
          if (this.checkWin(player)) {
            ...
            document.getElementById("lastMoveDisplay").textContent = 
              `${player.name} moved from ${previousPosition} to ${player.position}`;
            return;
          }
   ...
   return false;
    ...
    if (current != null) {
      if (currentPositionNode.endSquare < currentPositionNode.square) {
        // Snake
        snakeSound.play();  // 🐍 Play snake sound
        while (true) {
        ...
        // Ladder
        ladderSound.play();  // 🪜 Play ladder sound
        while (true) {
          if (current.square == currentPositionNode.endSquare) {
            player.position = current.square;
            return;
